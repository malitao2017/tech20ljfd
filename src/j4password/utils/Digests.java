...
public class Digests {
	...
	public static byte[] sha384(byte[] input) {
		return digest(input, SHA384, null, 1);
	}

	public static byte[] sha384(byte[] input, byte[] salt) {
		return digest(input, SHA384, salt, 1);
	}

	public static byte[] sha384(byte[] input, byte[] salt, int iterations) {
		return digest(input, SHA384, salt, iterations);
	}
